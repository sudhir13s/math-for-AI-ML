...
The chapter follows a deliberate progression. We start with **Fourier Series** (§01) — the discrete-spectrum decomposition of periodic functions — and develop the convergence theory, the energy identity (Parseval), and the Gibbs phenomenon. We then take the period to infinity to obtain the **Fourier Transform** (§02), establishing its properties, the uncertainty principle, and the Plancherel theorem. Section §03 discretizes everything into the **DFT and FFT**, the algorithm that makes Fourier analysis computable in $O(N \log N)$ time. Section §04 proves the **Convolution Theorem** — the single most important theorem in signal processing — and traces it through CNNs, state space models, and attention mechanisms. Finally, §05 introduces **Wavelets**, which overcome the Fourier transform's time-localization blindspot and underpin modern multi-scale architectures.
...
01-Fourier-Series            (trigonometric basis, Fourier coefficients — start here)
        ↓
02-Fourier-Transform         (continuous FT; builds directly on §01)
        ↓
03-DFT-and-FFT               (discretizes §02; Cooley-Tukey algorithm)
        ↓
04-Convolution-Theorem       (uses FT from §02 and DFT from §03)
        ↓
05-Wavelets                  (overcomes FT limitations; uses MRA and filter banks from §04)
        ↓
...
**§01 vs §02:** Fourier Series is for periodic functions; the Fourier Transform handles aperiodic signals. §02 is derived from §01 by letting the period $T \to \infty$ — the discrete sum of harmonics becomes a continuous integral. Parseval's theorem appears in both.

**§02 vs §03:** The DFT in §03 is the numerically computable version of the continuous FT in §02. The relationship is: sample a continuous signal at $N$ points → apply DFT → approximate the continuous spectrum. The FFT makes this feasible in $O(N \log N)$.

**§03 vs §04:** The Convolution Theorem in §04 is what makes the DFT from §03 practically useful. Circular convolution in the DFT domain becomes pointwise multiplication — reducing $O(N^2)$ convolution to $O(N \log N)$ FFT-based convolution.

**§04 vs §05:** The Convolution Theorem in §04 characterizes LTI systems. Wavelets in §05 use iterated convolution with filter banks (low-pass / high-pass) to build multi-scale signal decompositions. The perfect reconstruction conditions in §05 are the synthesis side of §04's filter theory.

**§01–§04 vs §05:** Fourier analysis gives global frequency information with no time localization. Wavelets sacrifice some frequency precision to gain time localization — a direct consequence of the uncertainty principle from §02.
...
|-------|---------------|---------------|
| Trigonometric Fourier series, Gibbs phenomenon | §01 | §02 (limit argument) |
| Fourier Transform, Plancherel, uncertainty principle | §02 | §03 (discretization), §04 (proof) |
| DFT matrix, FFT algorithm, spectral leakage | §03 | §04 (circular convolution) |
| Convolution theorem, cross-correlation, filter design | §04 | §03 (circular), §05 (filter banks) |
| Wavelet transform, MRA, Daubechies wavelets | §05 | §04 (filter bank connection) |
| $L^2$ Hilbert space theory | §12-02 (Hilbert Spaces) | §01 preview |
| Spectral graph convolution (GCN, ChebNet) | §11-04 (Spectral Graph Theory) | §04 brief reference |
| Kernel methods, RKHS, Bochner's theorem | §12-03 (Kernel Methods) | §02 brief reference |
...
- **$L^2$ function spaces** — inner product, norm, completeness ([Chapter 12](../12-Functional-Analysis/README.md), §02 Hilbert Spaces)

Sections §01–§03 need primarily calculus and complex analysis. Sections §04–§05 additionally need the abstract framework from §12.
